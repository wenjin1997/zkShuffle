...
export async function compile_circom(fileName: string, options: any) {
  ...
  try {
    ...
    // await execSync("circom " + fileName + flags + " --prime 12381");
...
export async function build_circuit(dir: string, circuit_name: string) {
  ...
  const final_ptau_file = P0X_DIR + "/zkey/bls12381.ptau";
  // await dnld_aws('zkey/ptau.20')
  await dnld_aws("zkey/bls12381.ptau");
  ...
  const ptau_final = {
    type: "mem",
    data: new Uint8Array(Buffer.from(fs.readFileSync(final_ptau_file))),
  };
  await generate_zkey_final_key(curve, ptau_final, r1cs_file, final_zkey_file);
  const templates = {
    groth16: await fs.promises.readFile(cwd + "/templates/verifier_groth16.sol.ejs", "utf8"),
  };
  ...
  verifierCode = verifierCode.replace(
    new RegExp("Pairing", "g"),
    titleCase(circuit_name) + "Pairing",
  );
...
export async function calculate_checksum(dir: string): Promise<Map<string, string>> {
  ...
  for await (const entry of directory) {
    ...
    if (entry.isDirectory()) {
      ...
      if (
        entry.name.substr(entry.name.length - 15) === "test.input.json" ||
        entry.name.substr(entry.name.length - 4) === ".png"
      ) {
        continue;
      }
      dict.set(
